...
class User(UserMixin, db.Model):
    ...
    profile_pic = db.Column(db.String(100), default='profile.png')
...
def unfollow(user_id):
    ...
    if current_user.is_following(user_to_unfollow):
        ...
        flash('You have unfollowed {}.'.format(
            user_to_unfollow.username), 'success')
    else:
...
def account():
    if request.method == 'POST':
        ...
        if profile_picture and allowed_file(profile_picture.filename):
            ...
            if current_user.profile_pic is not None and current_user.profile_pic != 'profile.png':
